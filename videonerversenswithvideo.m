...
vw.FrameRate=10/0.6;
vw_together = VideoWriter("neversens_together.avi");
...
vidObj = VideoReader("neversenswithvideo/00000.mp4");
ratio=87/540;
width=1158-702;
...
for k = 1:k_frame_inter:length(time4)
    ...
    while(hasFrame(vidObj) && vidObj.CurrentTime<videotime)
        ...
        if (vidObj.CurrentTime>videotime)
            ...
            ylim([-0.5,4.5]);
            xlim([-1,1]);
            plot(squeeze(r_all_SACS(2,1:plotinterval:n_points)), squeeze(r_all_SACS(3,1:plotinterval:n_points)),'m');
            hold on
