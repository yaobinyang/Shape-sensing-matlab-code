...
Times = readcell('data/newdata/ClaremontNorth36_N_tc.csv','range',"1:1");
strain_N=strain_N(2:end,:);
% strain_N=flip(strain_N,1);
strain_S=readmatrix("data/newdata/ClaremontNorth36_S_tc.csv");
strain_S=strain_S(2:end,:);
% strain_S=flip(strain_S,1);
strain_Z=readmatrix("data/newdata/ClaremontNorth36_Z_tc.csv");
...
vw = VideoWriter("simpsupportvideo","Motion JPEG AVI");
vw.FrameRate=1/0.6;
open(vw)
...
vw = VideoWriter("axialdispvideo","Motion JPEG AVI");
vw.FrameRate=1/0.6;
open(vw)
for slicetimeid=1:1:42
    figure(40001)
