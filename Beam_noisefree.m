...
function [r_all,D_ik_all,D0_ik_all,u0_k_all,v0_k_all,u_k_all,v_k_all,Cu_k_all,Cv_k_all,MSE_all]...
    =Beam_noisefree(R_helix,R_straight,omega_helix,omega_straight,theta_helix_0,theta_straight_0,n_helix,n_straight,s_all,rod_axis_rs,isassigneddir,assigned_dir,helix_cable_strain_all,straight_cable_strain_all,measure_error_level,plot_figure_id,n_substep)
...
for point=1:n_points
    ...
    if point>1
        ...
        helpermat_exp=eye(3)+Ucross_helpermat_trans*ds+0.5*(Ucross_helpermat_trans*Ucross_helpermat_trans)*ds^2;
        helpermat_int=eye(3)*ds+0.5*Ucross_helpermat_trans*ds^2+(1/6)*(Ucross_helpermat_trans*Ucross_helpermat_trans)*ds^3;
