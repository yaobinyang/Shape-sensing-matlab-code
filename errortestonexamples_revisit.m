...
name1 = ['errortestrevisit/errortest_longrod_nonconstall_largesample' strrep(datestr(now), ':', '-')];
%%
s_all=[0:1:100];
...
R_helix=0.01;R_straight=0.005;omega_helix=1;omega_straight=0;theta_helix_0=0;theta_straight_0=0;n_helix=6;n_straight=3;
% design_uk=[1;0.5;0.1];
% % design_vk=[-0.1;0.3;1.1];
% design_uk_all=0.5*[0.1;0.05;0.01]*ones(size(s_all))+1/pi*[-0.1;0.01;0.05]*sin(2*pi*s_all/s_all(end));
% design_vk_all=[0;0;1]*ones(size(s_all))+1e-2*[0;0;1]*ones(size(s_all))+1e-3*[1;0.2;0]*ones(size(s_all))+0*0.01*[-0.05;0.4;0.1]*ones(size(s_all))+1*1e-3*[0.5;0;0]*sin(2*pi*s_all/s_all(end))+1*1e-2/pi*[0;0;-1]*sin(2*pi*s_all/s_all(end));
design_uk_all=[0.1;0.05;0.01]*ones(size(s_all));
design_vk_all=[0;0;1]*ones(size(s_all))+-[0;0;0.1]+[0.05;0.2;0]*ones(size(s_all));
...
copyfile("errortestonexamples.m",name1,'f')