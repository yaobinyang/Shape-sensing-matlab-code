...
function [r_all,D_ik_all,D0_ik_all,u0_k_all,v0_k_all,u_k_all,v_k_all]...
    =Fully_nonlin_Cosserat_simulation(R_helix,R_straight,omega_helix,omega_straight,theta_helix_0,theta_straight_0,n_helix,n_straight,s_all,rod_axis_rs,isassigneddir,assigned_dir,helix_cable_strain_all,straight_cable_strain_all,measure_error_level,plot_figure_id,n_substep)
...
for point=1:n_points
    ...
    u0_k_all(:,point)=u0_k;
    v0_k_all(:,point)=v0_k;
    [A,B,u_k,v_k,Cu_k,Cv_k]=Fully_nonlin_nonlin_rod_assymbling_matrixs(R_helix,R_straight,omega_helix,omega_straight,theta_helix_0,theta_straight_0,n_helix,n_straight,s,u0_k,v0_k,epsilon_p_helix,epsilon_p_straight,measure_error_level,grad_s(point));
    u_k_all(:,point)=u_k;
    v_k_all(:,point)=v_k;
    % Cu_k_all(:,:,point)=Cu_k;
    % Cv_k_all(:,:,point)=Cv_k;
...
opts = odeset('RelTol',1e-12,'AbsTol',1e-12,'Refine',n_substep,'MaxStep',min(grad_s));
